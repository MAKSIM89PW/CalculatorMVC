...
class MainStackView: UIStackView {
    ...
    override init(frame: CGRect) {
        ...
        setupMainStackView()
        configure()
        stackViewsArrey.forEach { addArrangedSubview($0) }
    }
    ...
    private func setupMainStackView() {
        ...
        let buttonPluMinus = createButton(title: "+/-", tag: 17, color: .darkGray)
        let buttonPercent = createButton(title: "%", tag: 16, color: .darkGray)
        let buttonDevide = createButton(title: "/", tag: 15, color: .orange)
        let fifthStackView = UIStackView(subviews: [buttonAC, buttonPluMinus, buttonPercent, buttonDevide])
        
        stackViewsArrey = [fifthStackView, fourthStackView, thirdStackView, secondStackView, firstStackView]
    }
}
